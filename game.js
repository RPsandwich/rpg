...
// the game loop — runs ~60 times per second
    function gameLoop() {
      ...
      // // CUT CHECK — logic only, no drawing
      // cuttableBushes.forEach(function(bush) {
      //   if (!bush.cut && overlaps(player, bush) && keys[" "] && sword.collected) {
      //       bush.cut = true;
      //   }
      // });
      ...
      if (player.swinging) {
          player.swingTimer++;
          player.swingAngle += 0.1;
        if (player.swingTimer > 20 || player.swingAngle >= 2 * Math.PI / 3) {
      ...
      if (player.swinging) {
          ...
          const currentAngle = -Math.PI / 3 + player.swingAngle;
